...
import { Plus, Search, Calendar, CheckSquare, User, Edit2, Trash2 } from "lucide-react";
...
const TaskComponent = ({ setCurrentPage }) => {
  ...
  const deleteTask = async (taskId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/tasks/${taskId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const updateTask = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTasks(tasks.map(task => task.id === editingTask.id ? (data.task || data) : task));
